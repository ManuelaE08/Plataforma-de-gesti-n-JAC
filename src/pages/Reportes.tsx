    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Reportes generados</p>
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mt-1">En cola de generación</p>
        </div>
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Exportaciones</p>
          <p className="text-3xl font-bold text-[#2563EB]">12</p>
          <p className="text-xs text-gray-400 mt-1">PDF, Excel y CSV</p>
        </div>
      </div>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((item, index) => (
                  <tr key={index} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-600 tabular-nums">{item.fecha}</td>
                    <td className="px-4 py-3 text-gray-700 font-medium">{item.formato}</td>
                    <td className="px-4 py-3">
                      <Badge label={item.estado} variant={estadoVariant[item.estado]} />
                    </td>
                    <td className="px-4 py-3 text-gray-600">{item.generadoPor}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-gray-700 transition-colors" title="Ver">
                          <Eye size={15} />
                        </button>
                        <button className="p-1.5 rounded-lg hover:bg-blue-50 text-gray-400 hover:text-[#2563EB] transition-colors" title="Descargar">
                          <Download size={15} />
                        </button>
                      </div>
                    </td>
